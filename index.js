...
class ClusterManager extends EventEmitter {
    ...
    constructor(token, option) {
        ...
        this.webhook = typeof options.webhook === "string" ? {
            url: options.webhook,
            auth: undefined
        } : options.webhook
    ...
    // array
    get guilds() {
        return utils.getFromCollection(this.clusters, "guilds")
    }
    get groupChannels() {
        return utils.getFromCollection(this.clusters, "groupChannels")
    }
    get privateChannels() {
        return utils.getFromCollection(this.clusters, "privateChannels")
    }
    get relationships() {
        return utils.getFromCollection(this.clusters, "relationships")
    }
    get shards() {
        return utils.getFromCollection(this.clusters, "shards")
    }
    get unavailableGuilds() {
        return utils.getFromCollection(this.clusters, "unavailableGuilds")
    }
    get voiceConnections() {
        return utils.getFromCollection(this.clusters, "voiceConnections")
    }
    get users() {
        return utils.getFromCollection(this.clusters, "users")
    }

    // object
    get application() {
        return utils.getFromObject(this.clusters, "application")
    }
    get channelGuildMap() {
        return utils.getFromObject(this.clusters, "channelGuildMap")
    }
    get guildShardMap() {
        return utils.getFromObject(this.clusters, "guildShardMap")
    }
    get notes() {
        return utils.getFromObject(this.clusters, "notes")
    }
    get options() {
        return utils.getFromObject(this.clusters, "options")
    }
    get privateChannelMap() {
        return utils.getFromObject(this.clusters, "privateChannelMap")
    }
    get userGuildSettings() {
        return utils.getFromObject(this.clusters, "userGuildSettings")
    }
    get userSettings() {
        return utils.getFromObject(this.clusters, "userSettings")
    }
    ...
    _eventstowebhook() {
        ...
        let send = (type, data) => {
            return checkWebhook() ? this.fetch("POST", this.webhook.url, this.webhook.auth || false, {
                "type": type,
                "data": data,
                content: data ? data.content : undefined
            }) : undefined
        }
        this.on("start", () => send("start", {
            content: "Started!"
        }))
        this.on("clusterCreate", (bot) => send("clusterCreate", {
            bot: bot,
            content: `Cluster ${this.clusters.indexOf(bot)} | Ready!`
        }))

        this.on("connect", (id) => send("connect", {
            content: `Shard ${id} established connection!`,
            id: id
        }))
        this.on("shardDisconnect", (id, err) => send("shardDisconnect", {
            content: `Shard ${id} disconnected!`,
            id: id,
            message: err
        }))
        this.on("shardReady", (id) => send("shardReady", {
            content: `Shard ${id} is ready!`,
            id: id
        }))
        this.on("shardResume", (id) => send("shardResume", {
            content: `Shard ${id} has resumed!`,
            id: id
        }))
        this.on("warn", (message, id) => send("warn", {
            content: `Shard ${id} | ${message.stack}`,
            id: id,
            message: message
        }))
        this.on("error", (message, id) => send("error", {
            content: `Shard ${id} | ${message.stack}`,
            id: id,
            message: message
        }))
        this.on("ready", (bot, id, last) => send("ready", {
            content: `Shards ${id} - ${last} are ready!`,
            bot: bot,
            firstShardID: id,
            lastShardID: last
        }))
    ...
    fetch(method, url, auth, body, headers, responseType) {
        let authHeaders = {
            'authorization': auth || auth === false ? "" : this.token
        }
